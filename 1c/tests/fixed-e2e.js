...
class E2ETestRunner {
    ...
    setupBitrix24Mocking() {
        ...
        const webhook = '/rest/1/webhook123';

        // 🔍 Поиск сделки (не найдена)
        nock(baseURL)
            .persist()
            .post(`${webhook}/crm.deal.list`)
            .query(true)
            .reply(200, {
                result: [],
                total: 0
        ...
        nock(baseURL)
            .persist()
            .post(`${webhook}/crm.deal.productrows.set`)
            .reply(200, {
                result: true
            });
        ...
        nock(baseURL)
            .persist()
            .post(`${webhook}/crm.deal.update`)
            .reply(200, {
                result: true
            });

        // 🔍 Поиск существующей сделки (для обновления)
        nock(baseURL)
            .persist()
            .post(`${webhook}/crm.deal.list`)
            .query(query => query.filter && query.filter.includes('existing-deal'))
            .reply(200, {
                result: [{
                    ID: '123',
                    ...
                    COMPANY_ID: '101',
                    ASSIGNED_BY_ID: '1',
                    UF_CRM_GUID_ORDER: 'existing-deal-guid'
                }],
                total: 1
            });

        console.log('🎭 Bitrix24 моки настроены');
    }
    ...
    async errorHandling() {
        ...
        // Ожидаем ошибку, но корректную обработку
        ...
        if (response.status >= 400) {
            // Это тоже правильная обработка ошибки
            return;
        }

        throw new Error(`Error handling test failed: expected error but got ${response.status}`);
    }
    ...
    async runAllTests() {
        console.log('🧪 ЗАПУСК ПОЛНЫХ E2E ТЕСТОВ С МОКИНГОМ');
        console.log('============================================================');
